...
async function executeAdminAuth(e){
  ...
  try{
    ...
    showDashboardView();hydrateAdminProfile();await refreshAdminData();
  }catch(err){clearAdminSession();showLoginView();showError(err.message);}
}

async function refreshAdminData(){
  await Promise.allSettled([fetchAdminResults(),loadAdminExams()]);
...
function appendQuestionTemplate(type="MCQ"){
  ...
  if(type==="TF"){
    card.innerHTML=`<div class="question-card-head"><strong>Question ${questionCounter} · True / False</strong><button type="button" onclick="this.closest('.admin-question-card').remove()" class="question-remove-btn"><i data-lucide="x"></i></button></div>
      <div class="form-group"><label>Question Text</label><textarea class="q-text" required placeholder="Enter question"></textarea></div>
      <div class="form-group"><label>Correct Answer</label><select class="q-answer" required><option value="A">True</option><option value="B">False</option></select></div>
      <input class="q-a" type="hidden" value="True"><input class="q-b" type="hidden" value="False">`;
  }else{
    card.innerHTML=`<div class="question-card-head"><strong>Question ${questionCounter} · MCQ</strong><button type="button" onclick="this.closest('.admin-question-card').remove()" class="question-remove-btn"><i data-lucide="x"></i></button></div>
      <div class="form-group"><label>Question Text</label><textarea class="q-text" required placeholder="Enter question"></textarea></div>
      <div class="admin-options-grid">
        <div class="form-group"><label>Option A</label><input class="q-a" required></div><div class="form-group"><label>Option B</label><input class="q-b" required></div>
        <div class="form-group"><label>Option C</label><input class="q-c" required></div><div class="form-group"><label>Option D</label><input class="q-d" required></div>
      </div>
      <div class="form-group"><label>Correct Answer</label><select class="q-answer" required><option value="A">Option A</option><option value="B">Option B</option><option value="C">Option C</option><option value="D">Option D</option></select></div>`;
  }
  c.appendChild(card);refreshIcons();
}
...
async function createAdminExam(e){
  ...
  for(const path of candidates){
    ...
    if(res.ok){
      showNotice(responseData.message||"Exam created successfully.","success");e.target.reset();document.getElementById("dynamicQuestionsContainer").innerHTML="";questionCounter=0;closeCreateExamPanel();await loadAdminExams();return;
    }
...
function switchAdminTab(name,e){
  ...
  if(name==="analytics")fetchAdminResults();if(name==="exams")loadAdminExams();if(name==="profile")hydrateAdminProfile();refreshIcons();
}
