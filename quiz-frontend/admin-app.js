...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!adminJwtToken || localStorage.getItem('role') !== 'ADMIN') {
    clearAdminSession();
    showLoginView();
    return;
  }
...
function showDashboardView() {
  ...
  document.getElementById('adminEmailDisplay').textContent = localStorage.getItem('email') || 'Administrator';
  if (window.lucide) lucide.createIcons();
}
async function verifyAdminSession() {
  try {
    ...
    if (res.status === 403) { showDashboardView(); showError('Logged in as ADMIN, but the backend denied /admin/results. Check Spring Security authority configuration.'); return; }
    ...
    if (String(err.message).toLowerCase().includes('expired')) { clearAdminSession(); showLoginView(); } else { showDashboardView(); }
...
async function executeAdminAuth(e) {
  ...
  try {
    ...
    localStorage.setItem('name', data.name || 'Administrator');
    showDashboardView();
    await verifyAdminSession();
...
async function fetchAdminResultsWithSorting(){
  ...
  try{
    const res=await fetch(`${API}/admin/results`,{headers:{Authorization:`Bearer ${adminJwtToken}`}});
    if(res.status===401){clearAdminSession();showLoginView();throw new Error('Admin session expired.');}
    if(res.status===403){showError('Backend denied /admin/results for this ADMIN token.');return;}
...
function confirmAdminLogout(){ clearAdminSession(); localStorage.clear(); window.location.replace('admin.html'); }
