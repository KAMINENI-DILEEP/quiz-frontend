...
async function updateStudentPassword(e) {
    e.preventDefault();
    ...
    try {
        ...
        if (!res.ok) throw new Error(data.message || `Password update failed (HTTP ${res.status}).`);
        e.target.reset();
...
function toggleStudentPassword(id,e){
